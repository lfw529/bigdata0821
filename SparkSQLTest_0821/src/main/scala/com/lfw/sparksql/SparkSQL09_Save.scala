...
object SparkSQL09_Save {
  def main(args: Array[String]): Unit = {
    ...
    val df: DataFrame = spark.read.load("SparkSQLTest_0821/input/user.json")
    ...
    df.write.mode(SaveMode.ErrorIfExists).save("SparkSQLTest_0821/out")

    import spark.implicits._
    val ds: Dataset[String] = df.map(
      row => row.mkString(",")
    )
    ds.write.format("text").save("SparkSQLTest_0821/out2")
