...
import com.lfw.flink.bean.AvgVc;
import com.lfw.flink.bean.WaterSensor;
import com.lfw.flink.bean.WaterSensor2;
            ...
            public void open(Configuration parameters) throws Exception {
                aggregatingState = getRuntimeContext().getAggregatingState(new AggregatingStateDescriptor<Integer, AvgVc, Double>("agg-state", new AggregateFunction<Integer, AvgVc, Double>() {
                    ...
                    public AvgVc createAccumulator() {
                        return new AvgVc(0, 0); //封装Bean
                    }

                    @Override
                    public AvgVc add(Integer value, AvgVc accumulator) { //累加过程(内部)
                        return new AvgVc(accumulator.getVcSum() + value, accumulator.getCount() + 1);
                    }

                    @Override
                    public Double getResult(AvgVc accumulator) {  //求平均
                        return accumulator.getVcSum() * 1D / accumulator.getCount();
                    }

                    @Override
                    public AvgVc merge(AvgVc a, AvgVc b) { //外部合并
